...
let buttonOpacity = 0;
let buttonTick = 0;
// Track mouse position for button click //
...
(hh = h / 2),
  (opts = {
    ...
    strings: ["HAPPY", "BIRTHDAY!", "Jaadiii ❤️"],
    charSize: 30,
    charSpacing: 35,
    lineHeight: 40,
...
function anim() {
  ...
  if (showButton) {
    buttonTick++;
    let progress = Math.min(buttonTick / 60, 1);
    buttonScale = 1 + 0.1 * Math.sin(progress * Math.PI);
    buttonOpacity = progress;

    ctx.save();
    ctx.translate(hw, hh + 50);
    ctx.scale(buttonScale, buttonScale);
    ctx.globalAlpha = buttonOpacity;

    //Button Background
    let gradient = ctx.createLinearGradient(-110, -25, 110, 25);
    gradient.addColorStop(0, '#ff6b6b');
    gradient.addColorStop(1, '#ffd93d');
    ctx.fillStyle = gradient;
    ctx.shadowColor = 'rgba(255, 107, 107, 0.6)';
    ctx.shadowBlur = 20;
    roundRect(ctx, -110, -25, 220, 50, 25);
    ctx.fill();
